                ...
                <div className="space-y-4">
                    {automationRules.map((rule) => (
                        <div key={rule.id} className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-4">
                                    <div className={`p-3 rounded-xl ${rule.isActive ? 'bg-green-500/20' : 'bg-gray-500/20'}`}>
                                        <Zap className={`h-5 w-5 ${rule.isActive ? 'text-green-400' : 'text-gray-400'}`} />
                                    </div>
                                ...
                                <div className="flex items-center space-x-3">
                                    <span className={`px-2 py-1 rounded-full text-xs ${rule.tier === 'VIP' ? 'bg-yellow-500/20 text-yellow-200' :
                                            rule.tier === 'Premium' ? 'bg-purple-500/20 text-purple-200' :
                                                'bg-blue-500/20 text-blue-200'}`}>
                                        {rule.tier}
                                    </span>

                                    <button className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${rule.isActive
                                            ? 'bg-green-500/20 text-green-200 hover:bg-green-500/30'
                                            : 'bg-gray-500/20 text-gray-200 hover:bg-gray-500/30'}`}>
                                        {rule.isActive ? 'Active' : 'Inactive'}
                                    </button>
